...
function Navbar(props) {
    ...
    const volume = React.useState(.5)
    const volumeDisplay = React.useState(
    ...
    function setStartGame (){
        socket.emit("player-left-match")
        setLayout(<PickSuitsContent setLayout={setLayout} socket={socket} name={name} volume={volume}/>)
    ...
    return (
        <nav className="navbar--main">
            ...
            <ul className="navbar--options">
                ...
                <li className="navbar--volume" onClick={toggleVolume}>
                    {volumeDisplay[0]}
                </li>
            </ul>
        </nav>
